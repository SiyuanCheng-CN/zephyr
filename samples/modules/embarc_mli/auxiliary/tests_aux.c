					...
					mli_tensor *tensors[], uint32_t paths_num)
{
	...
	for (uint32_t idx = 0; idx < paths_num; idx++) {
		...
		if (descr.opened_file == NULL ||
		    ...
		    descr.data_type != IDX_DT_FLOAT_4B || descr.num_dim > MLI_MAX_RANK) {
			DEBUG_BREAK;
			ret = TEST_SUIT_ERROR;
			goto ret_label;
		}
		...
		while (elements_accounted < total_elements) {
			...
			if (idx_file_read_data(&descr, (void *)data, NULL) != IDX_ERR_NONE ||
			    mli_hlp_float_to_fx_tensor(data, descr.num_elements, tensors[idx]) !=
				    MLI_STATUS_OK) {
				DEBUG_BREAK;
				ret = TEST_SUIT_ERROR;
				...
				mli_tensor pred, ref_to_pred_output *out)
{
	...
	if (descr.num_elements != total_pred_elements || descr.num_dim != pred.rank) {
		DEBUG_BREAK;
		ret = TEST_FAILED;
		goto ret_label;
	}
	...
	const float quant_scale = (float)((int64_t)1l << mli_hlp_tensor_scale_shift(&pred)) /
				  (float)mli_hlp_tensor_scale(&pred, 0);
	...
	while (elements_accounted < total_pred_elements) {
		descr.num_elements = ((total_pred_elements - elements_accounted) < data_buf_size) ?
...
{
	if (target_tensor->el_type != MLI_EL_ASYM_I8 && target_tensor->el_type != MLI_EL_ASYM_I32) {
		DEBUG_BREAK;
		return TEST_FAILED;
	}
